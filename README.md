...
over CONUS + Mexico, 2000–present.

## Background

Natural hazards do not fire independently: a hurricane drives floods and severe
storms; drought conditions precede wildfire bursts. The question this project
asks is **what the cascade structure of natural hazards looks like** — which
event types trigger which others, with what time lag and what spatial
signature, and how stationary that structure is across years and regions.
...
graph, and recovering it interpretably is the headline output.
...
Harmonized hazard catalog over CONUS + Mexico (EONET + auxiliary sources),
...
baseline — a **+4.6 nats/event** improvement that is remarkably uniform across
five independent storm bursts:
...
The model's **spatial intensity field** tracks the storms it has never seen:
below is the predicted next-event intensity over CONUS at the peak of Hurricane
Francine (Sept 10 2024, ~18 h before landfall); white points are the actual
events that fired within ±12 h.

![Francine intensity field](docs/figures/case_study_spatial_heatmap.png)

**Calibration is honest about uncertainty:** the model is well-calibrated on
wildfires (the dominant mark) and pushes rare marks (earthquake, flood, severe
storm, tornado) into the low-likelihood tail rather than assigning them
confident wrong probabilities — which makes inverse log-likelihood usable as a
rare-hazard anomaly detector.
...
> with a *characterized* interpretability limit — both findings hold
> simultaneously.
